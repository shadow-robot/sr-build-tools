...
"circle") echo "Circle CI server"
  ...
  docker run -w "/root/sr-build-tools/ansible" -v /:/host:rw $docker_image  bash -c "git pull && git checkout $toolset_branch && sudo ansible-playbook -v -i \"localhost,\" -c local docker_site.yml --tags \"circle,$tags_list\" -e \"$extra_variables\" "
  ;;
...
"local") echo "Local run"
  ...
  docker run -w "/root/sr-build-tools/ansible" -v $HOME:/host:rw $docker_image  bash -c "git pull && git checkout $toolset_branch && sudo ansible-playbook -v -i \"localhost,\" -c local docker_site.yml --tags \"local,$tags_list\" -e \"$extra_variables\" "
  ;;
