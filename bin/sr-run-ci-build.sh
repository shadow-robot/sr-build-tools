...
"circle") echo "Circle CI server"
  ...
  docker run -w "/sr-build-tools/ansible" -v /:/host:rw $docker_image  git pull && sudo ansible-playbook -v -i "localhost," -c local docker_site.yml --tags "circle,$tags_list" -e "$extra_variables"
  ;;
