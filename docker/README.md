...
* -cn or --customername     Specify the name of the customer for AWS upload emails (default: Shadow customer)
...
### Customer Name (without spaces) ```-cn```

This allows the Docker container to store the name of the company or organisation (without spaces) using the hand so that when ROS LOGS are uploaded to AWS, the email that is sent automatically to Shadow Robot Company contains the name of the customer. This makes it easier for Shadow staff to figure out which customer is sending ROS LOGS. The user running the oneliner needs to enter a customer name using the -cn flag in the oneliner. For example, adding ```-cn Some_Company_Or_Organisation``` to the oneliner command will save the customer name inside the user's Docker container and enable ROS_Logs_Saver to email Shadow with the customer name: Some_Company_Or_Organisation
 
Presuming we do not want the driver to auto-launch for our example, the final command would be:

```bash
bash <(curl -Ls http://bit.do/launch-sh) -i shadowrobot/flexible-hand:kinetic-v0.2.28 -n flexible_hand_real_hw -e enp0s25 -r true -g false -l false -ck -cn Some_Company_Or_Organisation
```
