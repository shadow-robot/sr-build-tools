...
"local") echo "Local run"
  ...
  docker run -w "$docker_user_home/sr-build-tools/ansible" -e LOCAL_USER_ID=$(id -u) $docker_flags --rm -v $HOME:/host:rw $docker_image  bash -c "git pull && git checkout $toolset_branch && git pull && PYTHONUNBUFFERED=1 ansible-playbook -v -i \"localhost,\" -c local docker_site.yml --tags \"$tags_list\" -e \"$extra_variables\" "
  ;;
...
"local-docker") echo "Using Docker Image from Docker Hub"
  ...
  git pull && git checkout $toolset_branch && git pull && sudo PYTHONUNBUFFERED=1 ansible-playbook -v -i "localhost," -c local docker_site.yml --tags "$tags_list" -e "$extra_variables"
  ;;
